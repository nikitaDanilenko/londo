...
trait Tables {
  ...
  class Dashboard(_tableTag: Tag) extends profile.api.Table[DashboardRow](_tableTag, "dashboard") {
    ...
    lazy val userFk = foreignKey("dashboard_owner_id_fk", ownerId, User)(
      r => r.id,
  ...
  class DashboardEntry(_tableTag: Tag) extends profile.api.Table[DashboardEntryRow](_tableTag, "dashboard_entry") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(dashboardId), Rep.Some(projectId), Rep.Some(createdAt))).shaped.<>(
      { r => import r._; _1.map(_ => DashboardEntryRow.tupled((_1.get, _2.get, _3.get))) },
      (_: Any) => throw new Exception("Inserting into ? projection not supported.")
    )
  ...
  class LoginAttempt(_tableTag: Tag) extends profile.api.Table[LoginAttemptRow](_tableTag, "login_attempt") {
    def * = (userId, failedAttempts, lastSuccessfulLogin) <> (LoginAttemptRow.tupled, LoginAttemptRow.unapply)

    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(userId), Rep.Some(failedAttempts), lastSuccessfulLogin)).shaped.<>(
    ...
    /** Database column failed_attempts_since_last_successful_login SqlType(int4) */
    val failedAttempts: Rep[Int] = column[Int]("failed_attempts_since_last_successful_login")
  ...
  implicit def GetResultProjectRow(implicit
    ...
    import prs._
    ProjectRow.tupled(
      (<<[java.util.UUID], <<[java.util.UUID], <<[String], <<?[String], <<[java.sql.Timestamp], <<?[java.sql.Timestamp])
    )
  }
  ...
  class Task(_tableTag: Tag) extends profile.api.Table[TaskRow](_tableTag, "task") {
    ...
    def ? = (
      ...
      { r =>
        import r._;
        _1.map(_ => TaskRow.tupled((_1.get, _2.get, _3.get, _4, _5.get, _6.get, _7.get, _8.get, _9.get, _10)))
      },
      (_: Any) => throw new Exception("Inserting into ? projection not supported.")
    )
  ...
  class User(_tableTag: Tag) extends profile.api.Table[UserRow](_tableTag, "user") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? =
      ((Rep.Some(id), Rep.Some(nickname), Rep.Some(email), Rep.Some(salt), Rep.Some(hash), displayName)).shaped.<>(
        { r => import r._; _1.map(_ => UserRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6))) },
        (_: Any) => throw new Exception("Inserting into ? projection not supported.")
      )
