...
trait Tables {
  ...
  lazy val schema: profile.SchemaDescription = Array(Dashboard.schema, DashboardProjectAssociation.schema, LoginAttempt.schema, PlainTask.schema, Project.schema, ProjectReferenceTask.schema, SessionKey.schema, User.schema).reduceLeft(_ ++ _)
   ...
   *  @param publiclyVisible Database column publicly_visible SqlType(bool)
  ...
  case class DashboardRow(id: java.util.UUID, userId: java.util.UUID, header: String, description: Option[String] = None, publiclyVisible: Boolean, createdAt: java.sql.Date, updatedAt: Option[java.sql.Date] = None)
  /** GetResult implicit for fetching DashboardRow objects using plain SQL queries */
  implicit def GetResultDashboardRow(implicit e0: GR[java.util.UUID], e1: GR[String], e2: GR[Option[String]], e3: GR[Boolean], e4: GR[java.sql.Date], e5: GR[Option[java.sql.Date]]): GR[DashboardRow] = GR{
    prs => import prs._
    DashboardRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[String], <<?[String], <<[Boolean], <<[java.sql.Date], <<?[java.sql.Date]))
  }
  ...
  class Dashboard(_tableTag: Tag) extends profile.api.Table[DashboardRow](_tableTag, "dashboard") {
    def * = (id, userId, header, description, publiclyVisible, createdAt, updatedAt) <> (DashboardRow.tupled, DashboardRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(userId), Rep.Some(header), description, Rep.Some(publiclyVisible), Rep.Some(createdAt), updatedAt)).shaped.<>({r=>import r._; _1.map(_=> DashboardRow.tupled((_1.get, _2.get, _3.get, _4, _5.get, _6.get, _7)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val userFk = foreignKey("dashboard_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.NoAction)
  }
  ...
  implicit def GetResultDashboardProjectAssociationRow(implicit e0: GR[java.util.UUID]): GR[DashboardProjectAssociationRow] = GR{
    prs => import prs._
    DashboardProjectAssociationRow.tupled((<<[java.util.UUID], <<[java.util.UUID]))
  }
  /** Table description of table dashboard_project_association. Objects of this class serve as prototypes for rows in queries. */
  class DashboardProjectAssociation(_tableTag: Tag) extends profile.api.Table[DashboardProjectAssociationRow](_tableTag, "dashboard_project_association") {
    def * = (dashboardId, projectId) <> (DashboardProjectAssociationRow.tupled, DashboardProjectAssociationRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(dashboardId), Rep.Some(projectId))).shaped.<>({r=>import r._; _1.map(_=> DashboardProjectAssociationRow.tupled((_1.get, _2.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val dashboardFk = foreignKey("dashboard_project_association_dashboard_id_fk", dashboardId, Dashboard)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
    /** Foreign key referencing Project (database name dashboard_project_association_project_id_fk) */
    lazy val projectFk = foreignKey("dashboard_project_association_project_id_fk", projectId, Project)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
   ...
   *  @param failedAttemptsSinceLastSuccessfulLogin Database column failed_attempts_since_last_successful_login SqlType(int4)
   *  @param lastSuccessfulLogin Database column last_successful_login SqlType(timestamp), Default(None) */
  case class LoginAttemptRow(userId: java.util.UUID, failedAttemptsSinceLastSuccessfulLogin: Int, lastSuccessfulLogin: Option[java.sql.Timestamp] = None)
  /** GetResult implicit for fetching LoginAttemptRow objects using plain SQL queries */
  implicit def GetResultLoginAttemptRow(implicit e0: GR[java.util.UUID], e1: GR[Int], e2: GR[Option[java.sql.Timestamp]]): GR[LoginAttemptRow] = GR{
    prs => import prs._
  ...
  class LoginAttempt(_tableTag: Tag) extends profile.api.Table[LoginAttemptRow](_tableTag, "login_attempt") {
    def * = (userId, failedAttemptsSinceLastSuccessfulLogin, lastSuccessfulLogin) <> (LoginAttemptRow.tupled, LoginAttemptRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(userId), Rep.Some(failedAttemptsSinceLastSuccessfulLogin), lastSuccessfulLogin)).shaped.<>({r=>import r._; _1.map(_=> LoginAttemptRow.tupled((_1.get, _2.get, _3)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    val lastSuccessfulLogin: Rep[Option[java.sql.Timestamp]] = column[Option[java.sql.Timestamp]]("last_successful_login", O.Default(None))

    /** Foreign key referencing User (database name login_attempt_user_id_fk) */
    lazy val userFk = foreignKey("login_attempt_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
  ...
  case class PlainTaskRow(id: java.util.UUID, projectId: java.util.UUID, name: String, unit: Option[String] = None, kind: String, reached: Long, reachable: Long)
  /** GetResult implicit for fetching PlainTaskRow objects using plain SQL queries */
  implicit def GetResultPlainTaskRow(implicit e0: GR[java.util.UUID], e1: GR[String], e2: GR[Option[String]], e3: GR[Long]): GR[PlainTaskRow] = GR{
    prs => import prs._
    PlainTaskRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[String], <<?[String], <<[String], <<[Long], <<[Long]))
  }
  ...
  class PlainTask(_tableTag: Tag) extends profile.api.Table[PlainTaskRow](_tableTag, "plain_task") {
    ...
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(projectId), Rep.Some(name), unit, Rep.Some(kind), Rep.Some(reached), Rep.Some(reachable))).shaped.<>({r=>import r._; _1.map(_=> PlainTaskRow.tupled((_1.get, _2.get, _3.get, _4, _5.get, _6.get, _7.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val projectFk = foreignKey("plain_task_project_id", projectId, Project)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
   ...
   *  @param createdAt Database column created_at SqlType(date)
   *  @param updatedAt Database column updated_at SqlType(date), Default(None) */
  case class ProjectRow(id: java.util.UUID, ownerId: java.util.UUID, name: String, description: Option[String] = None, createdAt: java.sql.Date, updatedAt: Option[java.sql.Date] = None)
  /** GetResult implicit for fetching ProjectRow objects using plain SQL queries */
  implicit def GetResultProjectRow(implicit e0: GR[java.util.UUID], e1: GR[String], e2: GR[Option[String]], e3: GR[java.sql.Date], e4: GR[Option[java.sql.Date]]): GR[ProjectRow] = GR{
    prs => import prs._
    ProjectRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[String], <<?[String], <<[java.sql.Date], <<?[java.sql.Date]))
  }
   ...
   *  @param id Database column id SqlType(uuid)
   *  @param projectId Database column project_id SqlType(uuid)
   *  @param projectReferenceId Database column project_reference_id SqlType(uuid) */
  ...
  class ProjectReferenceTask(_tableTag: Tag) extends profile.api.Table[ProjectReferenceTaskRow](_tableTag, "project_reference_task") {
    def * = (id, projectId, projectReferenceId) <> (ProjectReferenceTaskRow.tupled, ProjectReferenceTaskRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(projectId), Rep.Some(projectReferenceId))).shaped.<>({r=>import r._; _1.map(_=> ProjectReferenceTaskRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    lazy val projectFk = foreignKey("project_reference_task_project_reference_id", projectId, Project)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
  ...
  /** Entity class storing rows of table SessionKey
   *  @param id Database column id SqlType(uuid), PrimaryKey
   *  @param userId Database column user_id SqlType(uuid)
   *  @param createdAt Database column created_at SqlType(date) */
  case class SessionKeyRow(id: java.util.UUID, userId: java.util.UUID, createdAt: java.sql.Date)
  /** GetResult implicit for fetching SessionKeyRow objects using plain SQL queries */
  implicit def GetResultSessionKeyRow(implicit e0: GR[java.util.UUID], e1: GR[java.sql.Date]): GR[SessionKeyRow] = GR{
    prs => import prs._
    SessionKeyRow.tupled((<<[java.util.UUID], <<[java.util.UUID], <<[java.sql.Date]))
  }
  /** Table description of table session_key. Objects of this class serve as prototypes for rows in queries. */
  class SessionKey(_tableTag: Tag) extends profile.api.Table[SessionKeyRow](_tableTag, "session_key") {
    def * = (id, userId, createdAt) <> (SessionKeyRow.tupled, SessionKeyRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(userId), Rep.Some(createdAt))).shaped.<>({r=>import r._; _1.map(_=> SessionKeyRow.tupled((_1.get, _2.get, _3.get)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
    ...
    /** Foreign key referencing User (database name session_key_user_id_fk) */
    lazy val userFk = foreignKey("session_key_user_id_fk", userId, User)(r => r.id, onUpdate=ForeignKeyAction.NoAction, onDelete=ForeignKeyAction.Cascade)
  }
  /** Collection-like TableQuery object for table SessionKey */
  lazy val SessionKey = new TableQuery(tag => new SessionKey(tag))

  /** Entity class storing rows of table User
   *  @param id Database column id SqlType(uuid), PrimaryKey
   *  @param nickname Database column nickname SqlType(text)
   *  @param email Database column email SqlType(text)
   *  @param passwordSalt Database column password_salt SqlType(text)
   ...
   *  @param description Database column description SqlType(text), Default(None) */
  case class UserRow(id: java.util.UUID, nickname: String, email: String, passwordSalt: String, passwordHash: String, displayName: Option[String] = None, description: Option[String] = None)
  ...
  class User(_tableTag: Tag) extends profile.api.Table[UserRow](_tableTag, "user") {
    def * = (id, nickname, email, passwordSalt, passwordHash, displayName, description) <> (UserRow.tupled, UserRow.unapply)
    /** Maps whole row to an option. Useful for outer joins. */
    def ? = ((Rep.Some(id), Rep.Some(nickname), Rep.Some(email), Rep.Some(passwordSalt), Rep.Some(passwordHash), displayName, description)).shaped.<>({r=>import r._; _1.map(_=> UserRow.tupled((_1.get, _2.get, _3.get, _4.get, _5.get, _6, _7)))}, (_:Any) =>  throw new Exception("Inserting into ? projection not supported."))
