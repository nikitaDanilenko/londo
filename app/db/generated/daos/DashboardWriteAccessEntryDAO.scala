...
class DashboardWriteAccessEntryDAO @Inject() (dbContext: DbContext, dbTransactorProvider: DbTransactorProvider) {
  ...
  def find[F[_]: Async: ContextShift](key: (UUID, UUID)): F[Option[DashboardWriteAccessEntry]] =
    findF(key).transact(dbTransactorProvider.transactor[F])

  def findF(key: (UUID, UUID)): ConnectionIO[Option[DashboardWriteAccessEntry]] = run(findAction(key)).map(_.headOption)
  ...
  def delete[F[_]: Async: ContextShift](key: (UUID, UUID)): F[DashboardWriteAccessEntry] =
    deleteF(key).transact(dbTransactorProvider.transactor[F])

  def deleteF(key: (UUID, UUID)): ConnectionIO[DashboardWriteAccessEntry] = run(deleteAction(key))
  ...
  private def findAction(key: (UUID, UUID)) =
    quote {
      PublicSchema.DashboardWriteAccessEntryDao.query.filter(a =>
        a.dashboardWriteAccessId == lift(key._1) && a.userId == lift(key._2)
      )
    }
